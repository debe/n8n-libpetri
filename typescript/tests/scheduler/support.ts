/**
 * Harness for the scheduler suite: a fake n8n `Workflow` built from the compiler fixtures
 * (`tests/fixtures/workflows.ts`), a `FakeHost` implementing the 30 `SchedulerHost`
 * members with canned `runNode` outputs per node, fake lifecycle hooks, and a recorder of
 * every host call and hook in order (`calls`). The host methods mirror what
 * `WorkflowExecute` does at n8n `441970b` closely enough for `runData`, `source` and
 * `pairedItem` to come out in n8n's shape; the recorder is what the ordering tests read.
 */
...
import type {
  ...
  INodeParameters, IPairedItemData, IRunData, IRunExecutionData, IRunNodeResponse, ITaskData, ITaskDataConnections,
  ITaskMetadata, ITaskStartedData, IWorkflowExecuteAdditionalData, Workflow, WorkflowExecuteMode, IPinData,
} from 'n8n-workflow';
import type { NodeDescription, WorkflowDescription } from '../../src/compiler/index.js';
import type { NodeHelpersLike, SchedulerHooks, SchedulerHost, WorkflowScheduler } from '../../src/n8n/host.js';
...
export async function execute(
  ...
  const host = new FakeHost(workflow, runExecutionData, scripts, options);
