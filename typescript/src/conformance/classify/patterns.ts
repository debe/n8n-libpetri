/**
 * The loop-driving rule's data: which files can hold a loop-driving case, and the describe
 * blocks (and, where a block is mixed, the titles) that select one. `classify.ts` applies it;
 * the counts each pattern yields on the unpatched baseline are pinned in
